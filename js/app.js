...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ==========================================
  // 11. 全站瀏覽人次計數器 (Visitor Counter)
  // ==========================================
  function initVisitorCounter() {
    let totalVisits = parseInt(localStorage.getItem('ocean_craftsman_visits') || '0', 10);
    if (!totalVisits || isNaN(totalVisits) || totalVisits < 1280) {
      totalVisits = 1288;
    }
    // 每次造訪增加 1 次
    totalVisits += 1;
    localStorage.setItem('ocean_craftsman_visits', totalVisits.toString());
    ...
    if (lastDate === todayStr && todayVisits > 0) {
      ...
      todayVisits = 48 + Math.floor(Math.random() * 5);
    ...
    if (el.navVisitorBadge) {
      el.navVisitorBadge.addEventListener('click', () => {
        showToast(`✨ 感謝蒞臨！您是本百科第 ${formattedTotal} 位水族探索鑑賞家！`);
      });
    }
  }
  ...
  function initCompatibilityMatcher() {
    // 1. 填入 50 種魚類下拉選單 (按分類分組)
    if (el.compatSelect) {
      const categories = [
        { key: 'corydoras', label: '鼠魚深度專題 (Corydoradinae)' },
        { key: 'flagship', label: '職人特選旗艦名種 (Flagships)' },
        { key: 'cichlid', label: '南美短鯛與特色慈鯛 (Cichlids)' },
        { key: 'tetra', label: '精品加拉辛脂鯉 (Characidae)' }
      ];

      let optionsHtml = '<option value="">-- 請選擇欲混養的魚種 (共50種) --</option>';
      categories.forEach(cat => {
        const catSpecies = state.species.filter(s => s.category === cat.key);
